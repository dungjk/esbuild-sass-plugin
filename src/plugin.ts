...
export function sassPlugin(options: SassPluginOptions = {}): Plugin {
  ...
  if (options.includePaths) {
    console.log(
      `'includePaths' option is deprecated, please use 'loadPaths' instead`
    );
  }
  ...
  return {
    ...
    async setup({
      ...
      const transform = options.transform
        ? options.transform.bind(options)
        : null;
      ...
      const renderAsync = createRenderer(
        compiler,
        options,
      ...
      onLoad(
        ...
        useCache(options, fsStatCache, async (path) => {
          try {
            ...
            if (transform) {
              const out: string | OnLoadResult = await transform(
                cssText,
                resolveDir,
                path
              );
              if (typeof out !== "string") {
                ...
                if (type === "css") {
                  ...
                  return {
                    errors: [
                      {
                        text: `unsupported type '${type}' for postCSS modules`,
                      },
                    ],
                  };
                }
                ...
                let exportConstants = "";
                if (options.namedExport && pluginData.exports) {
                  const json = JSON.parse(pluginData.exports);
                  const getClassName =
                    typeof options.namedExport === "function"
                      ? options.namedExport
                      : ensureClassName;
